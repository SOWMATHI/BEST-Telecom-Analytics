...
import {
  ...
  DialogActions, Grid, Alert, LinearProgress, Avatar, MenuItem,
} from "@mui/material";
import { DataGrid } from "@mui/x-data-grid";
import { Search, Add, Edit, Delete, Visibility, Refresh, Storefront, LocationCity } from "@mui/icons-material";
import { getEmployees, createEmployee, updateEmployee, deleteEmployee } from "../../api/employees";
import { mockEmployees } from "../../api/mockData";
import { useBranch } from "../../context/BranchContext";

const EMPTY = { name: "", email: "", phone: "", designation: "", department: "Wholesale Sales", branch: "Coimbatore", salary: "", monthlyTarget: 2000000 };

export default function Employees() {
  const { selectedBranch, currentBranch, filterByBranch } = useBranch();
  const [rows, setRows] = useState(mockEmployees);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const items = res.data?.employees || (Array.isArray(res.data) ? res.data : []);
      if (Array.isArray(items) && items.length > 0) {
        const normalized = items.map((e, idx) => ({
          _id: e._id || `E${idx + 100}`,
          name: e.name || e.fullName || "Team Member",
          email: e.email || "staff@besttelecom.in",
          phone: e.phone || "+91 98765 00000",
          department: e.department || "Wholesale Sales",
          designation: e.designation || e.role || "Executive",
          branch: typeof e.branch === "object" ? e.branch?.branchName || e.branch?.name : e.branch || (idx % 2 === 0 ? "Coimbatore" : "Tirupur"),
          salary: e.salary || 35000,
          monthlySales: e.monthlySales || (idx + 1) * 350000,
          target: e.target || e.monthlyTarget || 2000000,
        }));
        setRows(normalized);
      } else {
        setRows(mockEmployees);
      }
    } catch {
      setRows(mockEmployees);
    } finally {
  ...
  const filtered = branchFiltered.filter((r) =>
    [r.name, r.fullName, r.email, r.department, r.designation, r.branch].some((f) =>
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 0.9,
      renderCell: (p) => (
        <Chip label={p.value || "Wholesale Sales"} size="small" sx={{ bgcolor: "rgba(255, 255, 255, 0.06)", color: "#FFFFFF", fontWeight: 700, fontSize: 11, border: "1px solid rgba(255, 255, 255, 0.1)" }} />
      ),
    },
    { field: "designation", headerName: "Role / Designation", flex: 1, renderCell: (p) => <Typography variant="body2" color="#94A3B8">{p.value || p.row.role || "Executive"}</Typography> },
    {
      field: "monthlySales",
      headerName: "Monthly Target",
      flex: 1.1,
      renderCell: (p) => {
        const achieved = p.value || 0;
        const target = p.row.target || p.row.monthlyTarget || 2000000;
        const pct = Math.min(Math.round((achieved / target) * 100), 100);
        return (
          <Box sx={{ width: "100%", pr: 2 }}>
            <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
              <Typography variant="caption" color="#00E599" fontWeight={700}>₹{(achieved / 100000).toFixed(1)}L</Typography>
              <Typography variant="caption" color="#94A3B8">{pct}%</Typography>
            </Box>
            <LinearProgress variant="determinate" value={pct} sx={{ height: 5, borderRadius: 3, bgcolor: "rgba(255,255,255,0.06)", "& .MuiLinearProgress-bar": { bgcolor: "#00E599", borderRadius: 3 } }} />
          </Box>
        );
      },
    },
    {
      ...
      renderCell: (p) => (
        <Box sx={{ display: "flex", gap: 0.5 }}>
          <Tooltip title="View"><IconButton size="small" onClick={() => setViewItem(p.row)} sx={{ color: "#94A3B8" }}><Visibility fontSize="small" /></IconButton></Tooltip>
          <Tooltip title="Edit"><IconButton size="small" onClick={() => { setForm({ name: p.row.name || p.row.fullName || "", email: p.row.email || "", phone: p.row.phone || "", designation: p.row.designation || p.row.role || "", department: p.row.department || "", branch: p.row.branch || "Coimbatore", salary: p.row.salary || "", monthlyTarget: p.row.monthlyTarget || p.row.target || 2000000 }); setEditId(p.row._id); setDialogOpen(true); }} sx={{ color: "#00E599" }}><Edit fontSize="small" /></IconButton></Tooltip>
    ...
    <Box>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3, flexWrap: "wrap", gap: 2 }}>
        <Box>
          ...
          <Typography variant="body2" color="#94A3B8">{filtered.length} team members assigned across wholesale hubs</Typography>
        </Box>
      ...
      <Card sx={{ borderRadius: 3, border: "1px solid rgba(255, 255, 255, 0.08)", boxShadow: "0 4px 20px -2px rgba(0, 0, 0, 0.7)", mb: 2.5, bgcolor: "#0D131F" }}>
        <Box sx={{ p: 2 }}>
          <TextField fullWidth size="small" placeholder="Search by staff name, department, role, branch..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3, p: 1, bgcolor: "#0D131F", border: "1px solid rgba(0, 229, 153, 0.2)" } }}>
        <DialogTitle sx={{ fontWeight: 800, color: "#FFFFFF" }}>{editId ? "Edit Staff Member" : "Add Staff Member"}</DialogTitle>
        <DialogContent>
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            {[["name","Full Name"],["email","Email Address"],["phone","Phone Number"],["department","Department"],["designation","Designation / Role"],["salary","Monthly Salary (₹)"],["monthlyTarget","Monthly Target (₹)"]].map(([field, label]) => (
              <Grid item xs={12} sm={6} key={field}>
                <TextField fullWidth size="small" label={label} type={field === "salary" || field === "monthlyTarget" ? "number" : "text"} value={form[field]} onChange={(e) => setForm((f) => ({ ...f, [field]: e.target.value }))} sx={{ "& .MuiOutlinedInput-root": { borderRadius: 2, bgcolor: "#070A0E" } }} />
              </Grid>
            ))}
      ...
      <Dialog open={Boolean(viewItem)} onClose={() => setViewItem(null)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 3, p: 1, bgcolor: "#0D131F", border: "1px solid rgba(0, 229, 153, 0.2)" } }}>
        <DialogTitle sx={{ fontWeight: 800, color: "#FFFFFF" }}>Staff Profile</DialogTitle>
        <DialogContent>
          {viewItem && (
            <Grid container spacing={1.5} sx={{ mt: 0.5 }}>
              {[["Full Name", viewItem.name || viewItem.fullName], ["Assigned Hub", viewItem.branch || "Coimbatore"], ["Email", viewItem.email], ["Phone", viewItem.phone], ["Department", viewItem.department], ["Designation", viewItem.designation || viewItem.role], ["Monthly Salary", `₹${Number(viewItem.salary || 0).toLocaleString("en-IN")}`], ["Sales Target", `₹${Number(viewItem.monthlyTarget || viewItem.target || 0).toLocaleString("en-IN")}`]].map(([k, v]) => (
      ...
      <Dialog open={Boolean(deleteId)} onClose={() => setDeleteId(null)} PaperProps={{ sx: { borderRadius: 3, p: 1, bgcolor: "#0D131F", border: "1px solid rgba(239, 68, 68, 0.3)" } }}>
        <DialogTitle sx={{ fontWeight: 800, color: "#FFFFFF" }}>Delete Staff Member</DialogTitle>
        <DialogContent><Typography color="#94A3B8">Are you sure you want to delete this staff record?</Typography></DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
